...
def main(case_number,support,input_path,output_path,p=0.9):

	rdd = readfile(input_path)
	p = 1/rdd.getNumPartitions()
	s = max(1,math.floor(p*support))
	...
	def A_prior(input):
		def epoch(content,prior,num,s,pair_map):
		...
		i = 2
		while(True):
			pos_prior = epoch(content,prior,i,s,pair_map)
	...
	def PCY(input):
		...
		for List in content:
			...
			for i in range(len(b_id)):
				...
				for j in range(i+1,len(b_id)):
					# index = hash(b_id[i]+b_id[j]) % 10
					index = hash_fun([b_id[i],b_id[j]],n_buckets,String=True)
					pair_map[index] = pair_map.get(index,0)+1
					for k in range(j+1, len(b_id)):
						index_trip = hash_fun([b_id[i],b_id[j],b_id[k]],n_buckets,String = True)
		...
		for key in single_map.keys():#### generate bit map for single 
			if(single_map[key]>=s):
				...
				single_map[key] = 0
		for key in pair_map.keys(): ### generate bit map ###
			if(pair_map[key]>=s):
				pair_map[key] = 1
			else:
				pair_map[key] = 0
		...
		# 	temp = []
		# 	result = []
		# 	getSubset(b_id,temp,result,0,-1,pair_map,n_buckets)
			...
			for i in range(1,len(array)+1):
				for element in combinations(array,i):
					...
					if(i==2):
						...
						if(trip_map[hash_fun(element,n_buckets,String=True)]==1):
							final_map[key] = final_map.get(key,0)+1
					else:
						final_map[key] = final_map.get(key,0)+1
		# print(final_map)
	...
	def sort(x):
		...
		return tuple(a)
	if(case_number==1):
		rdd_cleaned = rdd.filter(f).map(split_fun).map(tuple).distinct().groupByKey().mapValues(list).map(lambda x: (x[0],sorted(x[1])))
	else:
		rdd_cleaned = rdd.filter(f).map(split_fun).map(lambda x:(x[1],x[0])).map(tuple).distinct().groupByKey().mapValues(list).map(lambda x: (x[0],sorted(x[1])))

	rdd_cleaned_A_priori = rdd_cleaned.mapPartitions(A_prior).reduceByKey(add)
...
start = time.time()
main(case_number,support,input_path,output_path)
end = time.time()
